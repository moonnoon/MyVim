...
if v:version < 700
  echohl ErrorMsg | echom "bufkill.vim requires vim version 7.00 or greater (mainly because it uses the new lists functionality)" | echohl None
  finish
endif
...
" The main function that sparks the buffer change/removal
  if !exists('w:BufKillList')
    echohl ErrorMsg | echom "BufKill Error: array w:BufKillList does not exist!" | echohl None
    echohl ErrorMsg | echom "Restart vim and retry, and if problems persist, notify the author!" | echohl None
    return
  endif
  ...
  if &modified && strlen(a:bang) == 0
    ...
    if s:BufKillActionWhenModifiedFileToBeKilled =~ '[Ff][Aa][Ii][Ll]'
      echohl ErrorMsg | echom "No write since last change for buffer '" . bufname(s:BufKillBufferToKill) . "' (add ! to override)" | echohl None
      return
    elseif s:BufKillActionWhenModifiedFileToBeKilled =~ '[Cc][Oo][Nn][Ff][Ii][Rr][Mm]'
      ...
      if action + actionAdjustment == 1
        ...
        silent w
        if v:errmsg != ""
          echohl ErrorMsg | echom "Unable to write buffer!" | echohl None
          return
        endif
      ...
      echohl ErrorMsg | echom "Illegal value (' . s:BufKillActionWhenModifiedFileToBeKilled . ') stored in variable s:BufKillActionWhenModifiedFileToBeKilled, please notify the author" | echohl None
    endif
  endif
...
function! <SID>UndoKill() "{{{1
  ...
  if !exists('s:BufKillLastBufferKilledNum') || !exists('s:BufKillLastBufferKilledPath') || s:BufKillLastBufferKilledNum == -1 || s:BufKillLastBufferKilledPath == ''
    echohl ErrorMsg | echom 'BufKill: nothing to undo (only one level of undo is supported)' | echohl None
  else
    ...
    elseif filereadable(s:BufKillLastBufferKilledPath)
      ...
      echohl ErrorMsg | echom 'BufKill: unable to undo. Neither buffer (' . s:BufKillLastBufferKilledNum . ') nor file (' . s:BufKillLastBufferKilledPath . ') could be found.' | echohl None
    endif
